defmodule CostcoMallEe.Mall do
  @moduledoc """
  The Mall context.
  """

  import Ecto.Query, warn: false
  alias CostcoMallEe.Repo
  ...
  def get_carts_count_total!(count_gt, total_gt) do
    query = from c in Cart,
      where: c.count > ^count_gt and c.total > ^total_gt,
      select: c

    Repo.all(query)
  end

  def get_carts_count!(count_gt) do
    query = from c in Cart,
      where: c.count > ^count_gt,
      select: c

    Repo.all(query)
  end

  def get_carts_total!(total_gt) do
    query = from c in Cart,
      where: c.total > ^total_gt,
      select: c
